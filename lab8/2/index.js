...
app.get('/signin', (req, res) => {
    ...
    const { loginType, identifier, password } = req.query;

    if (!loginType || !identifier || !password) {
        return res.render("signin"); // แสดงหน้า form
    }
    ...
    conn.query(sql, [identifier], (err, results) => {
        ...
        if (results.length === 0) { //คืนกลับมาเป็น empty array
            return res.render("accountNotFound");
        }

        const user = results[0]; //เอา result ที่เจออันแรกไปแสดงใน profile
